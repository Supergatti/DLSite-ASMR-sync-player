...
一个轻量的本地网页播放器。可在浏览器中同时播放所选目录或远程网盘（Alist）中的音频、视频和图片文件。
...
- 可在“视频 / 图片轮播”两种模式间切换；图片支持顺序或随机循环、上一张/下一张和切换间隔
- 图片轮播使用独立计时器，音频播放、暂停和进度跳转均不会改变图片轮播
- 图片模式使用紧凑单行控件；画面左右透明区域可切换图片，并支持左右滑动、双指缩放、放大后拖动和鼠标滚轮/双击缩放
- 视频和音频支持后退/前进 10 秒、顺序播放下一项，以及“循环 / 自动下一项 / 播放后停止”结束行为
- 底部按钮分别控制视频/图片与音频，鼠标停在控制栏上时不会自动隐藏
- 可从局域网设备浏览 PC 上配置好的媒体库
- 通过受限目录 API 流式传输文件，支持浏览器 Range 请求和进度跳转
- 触屏设备单击画面显示或隐藏控制栏；画面点击不会改变播放状态
...
## 支持格式
- 视频: `.mp4`, `.webm`, `.mkv`, `.ogg`
- 图片: `.jpg`, `.jpeg`, `.png`, `.webp`, `.gif`, `.avif`, `.bmp`
